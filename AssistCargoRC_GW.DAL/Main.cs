...
namespace AssistCargoRC_GW.BLL
{
    /// <summary>
    ...
    public static class Main
    {
        ...
        public static void StartUp()
        {
            ...
            CheckInstanceId();
             ...
             while (!DataOut.CheckTokenFromCache())
            {
                Thread.Sleep(5000);
                continue;
            }
        ...
        {
            Feedback.Log("Looking for Xmls to send.", false, 0);

            if (Cache.XmlsReadyToSend())
            {
                if (Cache.IsSending == true)
                    Feedback.Log("Another thread is currently sending. Starting a new one.", true, 0);
                else
                    DataOut.Send();
            }
            else
            {
                if (Cache.IsSending == true)
                    Feedback.Log("Another thread is currently sending, and there's no more Xmls to send.", true, 0);
                else
                    Feedback.Log("There's no Xmls to send.", true, 0);
            }
        }
        public static void CheckVehiclesList()
        {
            try
            {
                if (Configurations.CheckOrSetVehiclesList())
                {
                    ...
                    Feedback.Log("Looking for changes in vehicles list.", false, 0);
                    Feedback.Log("Changes were found in the vehicles list, now the application have: " + cant + " vehicles.", true, 1);
                }
                else
                {
                    Feedback.Log("Looking for changes in vehicles list.", false, 0);
                    Feedback.Log("No changes were found in the vehicles list.", true, 0);
                }
            }
            catch (Exception ex)
            {
        ...
        public static bool DbVerifications(int origin)
        {
            ...
            Helpers.GenericVerification("Incorrect DBForwarding Address configuration, remaining attempts: ",
                CheckDBForwardingAddressConfig, null, "AssistCargoRC Address configuration: \n Ip - " + Cache.Instance.IPAddressIn +
                                      "\n Port - " + Cache.Instance.PortIn + "\n Account ID - " + AccountsList()
                                      + "\n Api client url set: " + Cache.ApiClient.UrlBase);

            Helpers.GenericVerification("Incorrect DBForwarding Items configuration, ", null, VerifyDBForwardingItems, null, "DBForwarding configured correctly.");
        ...
        private static bool CheckDBForwardingAddressConfig()
        {
            return DAL.Configuration.CheckDBForwardingAddress(Cache.Instance.IPAddressIn, Cache.Instance.PortIn, Cache.Instance.Account);
        }
        private static string VerifyDBForwardingItems()
        {
            return DAL.Configuration.CheckDBForwardingItems(DTO.Properties.Settings.Default.ProtocolId, Cache.Instance.IPAddressIn, Cache.Instance.PortIn, Cache.Instance.Account);
        }
